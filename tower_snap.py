...
def proximity_max_m() -> float:
    ...
    try:
        ...
        logger.warning(
            "Invalid ORCHESTRATOR_PROXIMITY_MAX_M=%r â€” using %s m",
            raw,
    ...
    *,
    ...
    if connection is None:
        try:
            ...
            own_sql = True
        except Exception as exc:  # noqa: BLE001 â€” keep classify running
            logger.warning(
                "Tower DB snap skipped â€” could not connect to Azure SQL: %s",
                exc,
            )
    ...
    logger.info(
        "TOWER SNAP â€” checking %d net-new site(s) vs FCC/TowerSource within %.0f m",
    ...
    try:
        ...
        for index, canonical in classify_targets:
            ...
            try:
                ...
                stats["errors"] += 1
                logger.warning(
                    "Tower snap skipped â€” missing lat/lng for %s",
            ...
            stats["snapped"] += 1
            message = (
                f"snapped {hit.distance_m:.1f} m â†’ {hit.source}"
    ...
    logger.info(
        "TOWER SNAP â€” snapped=%d  no_hit=%d  errors=%d  checked=%d",
        stats["snapped"],
        stats["no_hit"],
        stats["errors"],
        stats["checked"],
